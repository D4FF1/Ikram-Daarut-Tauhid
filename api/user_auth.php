...
if ($method === 'GET') {
    if ($action === 'me') {
        ...
        // User info is taken from last known registration.
        $stmt = $conn->prepare(
            "SELECT nama, email, no_hp, asal_institusi FROM registrations WHERE email = ? ORDER BY created_at DESC LIMIT 1"
        );
        ...
        // If no registration yet, still allow login by returning basic info
        $user = [
            'nama' => $row['nama'] ?? '',
            'email' => $email,
            'no_hp' => $row['no_hp'] ?? '',
